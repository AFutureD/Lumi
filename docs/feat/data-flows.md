...
> 验证状态：开发预览。daemon、Mac 与 iOS 模拟器已通过同一合成 Session 的创建、时间线同步和删除验证；模型配置、内部上下文和消耗指标已通过结构化解析、保存与跨端编译验证；推送链路三端有单元测试。真实 Codex Hook 长期运行与 iPhone 实机仍待验收。
...
- **隐私边界**：Session 内容按原样完整保留——消息与 reasoning 全文、工具的完整输入与输出（点击 Activity 行即可查看原始数据）、每类最新内部上下文的完整嵌套内容（基础指令、线程上下文、世界状态、压缩历史）。其中可能出现路径、凭据、环境信息。本地副本及已配对设备都必须视为高敏感数据；当前没有内容级脱敏保证。Lumi 未识别的其他事件和完整原始日志文件不会被另外复制。
...
4. daemon 把同一条事件按目标 iPhone 加密，经 Relay 转发（Mac App 是否打开无关）；回合结束、失败或中断时另发一条明文标题的推送提醒。
5. iPhone 用与 daemon 相同的规则更新本机缓存和界面；对不上的地方用 Session 索引对账补齐。

Mac 界面的外部内容只在三种时机同步：App 启动、用户手动刷新、Agent 事件。删除、清空、标记已查看和 Notch 归档会立即同步操作结果；手动刷新的重算保留这两个人为标记。规则见 [MAC-R-009](modules/mac-session-view.md#mac-r-009-mac-界面只有三种同步入口)。

### 删除

- **单条删除**：用户在 Mac 选择 Session，通过删除图标（Delete Session）并确认后，daemon、Mac 和已连接 iPhone 移除同一 Session（当时不在线的 iPhone 在重连对账时移除）。删除 Main Session 会连带删除挂在它下面的全部 Subagent（含 Subagent 的 Subagent）；单独删除一个 Subagent 不影响父级。
- **全部清空**：用户在“Settings > Daemon”确认清空后，全部 Lumi Session 被清空。
- **删除后的新事件**：被动到达的旧活动不会让已删除 Session 重新出现；同一 Session 再次被人使用（新的用户请求，或会话重新启动）时它会回来。
- **外部影响**：Agent 自身 Session 和日志不被删除。
...
| iPhone App | 每台已配对 Mac 一个本机缓存数据库；配对凭据在 Keychain（[IOS-R-010](modules/iphone-live-view.md#ios-r-010-session-内容缓存在本机)） | 启动立即显示缓存；Mac 离线仍可翻看 |
| Relay | 设备授权、进行中的配对会话、每台设备的推送地址、传输进度和限流等运行所需信息 | 不提供 Session 正文或历史查询；推送提醒的标题和状态不写入存储 |

## 离线与恢复

- **daemon 离线**：Mac 保留已同步内容供查看，但“Settings > Daemon”显示 Not connected；恢复后可手动刷新。
...
- **主要消费者**：Lumi for Mac 的更新提示、“Settings > About”的版本显示和 Updates 状态行。
...
1. 自动调度或用户主动检查读取正式更新信息并验证其签名。
2. Lumi 比较当前 build 与可用 build；没有更高版本时报告当前已是最新。
3. 有新版本时，用户决定是否继续；更新包必须通过来源和完整性验证。
4. 用户确认安装后 App 被完整替换并重新启动，Session 历史保留；已安装的 helper 与 daemon 在下次启动时自动刷新（[UPD-R-006](modules/software-updates.md#upd-r-006-app-更新后刷新本机组件)）。
