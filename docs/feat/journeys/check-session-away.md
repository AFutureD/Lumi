...
> 验证状态：开发预览。iOS 模拟器已与真实 Mac 经 Relay 配对并同步 Session，合并列表、详情、Macs 和 Settings 均已走查；2026-08-23 起配对改为“6 位配对码 + 两端比对数字 + Mac 点 Match”，Relay、daemon、iOS 三端都有单元测试，但尚未在真机 iPhone 上端到端验收；弱网恢复同样待验收。
...
- Mac 的 daemon 和网络可用（Mac App 只在配对时需要打开）。
- iPhone 能访问这台 Mac 用的 Relay：内置 Relay 不用填；自托管时扫码自动带上，手输要在 Advanced 里填。
...
3. 回到 Sessions Tab。
   - 系统反馈：两台 Mac 的 Session 合并在一条列表里；搜索栏下方的 `Macs` 下拉里多出这台 Mac；Macs 页每行写着各自的 Relay 地址。
...
- 用户看到：Mac“iPhone”页显示 Relay unavailable，没有配对码。
- 可执行动作：确认网络可用；Mac 侧 Relay 地址由构建固定，App 内无法修改。
...
- 用户看到：连接关闭；Macs 页这台 Mac 显示 `Revoked · <Relay 地址>`，缓存仍可翻看。
...
- Relay 保存授权、进行中的配对会话和运行所需信息，不保存可浏览的 Session 历史。
