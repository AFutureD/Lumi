...
Notch 紧凑状态统计全部符合展示条件的 Session，展开后显示最近更新的最多六个主 Session 的标题、状态和当前 Turn 用户消息；Session 完成时可以短暂显示完成卡片。Notch 设置按钮打开主 App 的“Settings > Notch”，用户可以在第三栏的 Appearance section 选择显示屏幕，调整并恢复紧凑宽度、展开宽度和展开动画；Theme 固定为 Dark，Layout 固定为 Notch，但不显示这两个控件。
