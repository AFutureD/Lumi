...
Mac 主窗口参考系统 Mail.app：左侧是功能导航，中间是当前区域的列表，右侧是详情。Sessions 使用可折叠 Outline，Subagent 按父子关系放在 Main Session 下；每行显示标题、Agent 和状态，详情按 Overview、Model Configuration、Usage、Internal Context、Activity 分模块展示。Settings 使用“设置分类—设置详情”；只有 iPhone 配对页会收起中栏。
