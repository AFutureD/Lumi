...
1. 在 Mac 侧边栏选择“iPhone”。
   - 系统反馈：页面显示 6 位配对码（如 `7KF-3QP`）、二维码、Relay 地址和倒计时；码 5 分钟到期作废，不自动换新，到期后点 New code 再出一个（[IOS-R-002](#ios-r-002-配对码短时且一次性)）。
...
- 行为：6 位配对码（字母 + 数字，不分大小写）5 分钟后失效，Mac 上停在 Expired、不自动换新，点 New code 或重新进入“iPhone”页才出新码；被一台 iPhone 用掉后立即作废；离开“iPhone”页或点 New code 也作废。
