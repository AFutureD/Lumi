...
- **隐私提示**：已配对设备会通过端到端加密通道收到 Session 的完整副本，其中包含标题、工作目录、活动时间线，以及保留的模型配置、内部上下文和消耗指标；内容在 iPhone 解密后写入本机同步数据库，因此只应配对受信任设备。敏感内容边界见[业务数据流](../data-flows.md#session-状态与时间线)。
...
- 限制或例外：配对凭据和本地同步副本保留；恢复后等待 Mac 重新推送完成。
...
iPhone 为每台 Mac 保留独立的当前同步副本，用于高效更新和恢复连接；主界面只展示活动时间线，模型配置、内部上下文和消耗指标的最新记录不混入活动列表。只有在收到在线状态且本轮同步完整后才展示 Session。用户移除该 Mac 通道时，对应同步内容一并删除。
