...
missing_daemon_stderr="$(printf '%s' '{"hook_event_name":"SessionStart","session_id":"missing-daemon"}' | \
  AGENT_STATUS_SOCKET="${smoke_root}/missing.sock" AGENT_STATUS_LOG_DIRECTORY="${log_directory}" "${binary_path}/agent-status-helper" 2>&1 >/dev/null)" || {
...
# Logs go next to the smoke database, never into ~/Library/Logs/Agent Status.
AGENT_STATUS_SUPPORT_DIRECTORY="${smoke_root}" \
AGENT_STATUS_SOCKET="${socket_path}" \
AGENT_STATUS_DATABASE="${database_path}" \
AGENT_STATUS_LOG_DIRECTORY="${log_directory}" \
AGENT_STATUS_LOG_LEVEL="debug" \
CODEX_HOME="${codex_home}" \
"${binary_path}/agent-status-daemon" 2>"${smoke_root}/daemon.stderr" &
daemon_pid=$!
...
printf '%s' '{"hook_event_name":"SessionStart","session_id":"smoke-session-0001","cwd":"/tmp/project"}' | \
  AGENT_STATUS_SOCKET="${socket_path}" AGENT_STATUS_LOG_DIRECTORY="${log_directory}" "${binary_path}/agent-status-helper"
...
printf '%s' "{\"hook_event_name\":\"SessionStart\",\"session_id\":\"smoke-ghost-0001\",\"cwd\":\"/tmp/project\",\"source\":\"startup\",\"transcript_path\":\"${ghost_transcript}\"}" | \
  AGENT_STATUS_SOCKET="${socket_path}" AGENT_STATUS_LOG_DIRECTORY="${log_directory}" "${binary_path}/agent-status-helper" --agent claude
