...
missing_daemon_stderr="$(printf '%s' '{"hook_event_name":"SessionStart","session_id":"missing-daemon"}' | \
  AGENT_STATUS_SOCKET="${smoke_root}/missing.sock" AGENT_STATUS_LOG_DIRECTORY="${log_directory}" "${binary_path}/Spark" 2>&1 >/dev/null)" || {
...
printf '%s' '{"hook_event_name":"SessionStart","session_id":"smoke-session-0001","cwd":"/tmp/project"}' | \
  AGENT_STATUS_SOCKET="${socket_path}" AGENT_STATUS_LOG_DIRECTORY="${log_directory}" "${binary_path}/Spark"
...
printf '%s' "{\"hook_event_name\":\"SessionStart\",\"session_id\":\"smoke-ghost-0001\",\"cwd\":\"/tmp/project\",\"source\":\"startup\",\"transcript_path\":\"${ghost_transcript}\"}" | \
  AGENT_STATUS_SOCKET="${socket_path}" AGENT_STATUS_LOG_DIRECTORY="${log_directory}" "${binary_path}/Spark" --agent claude
